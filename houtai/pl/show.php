...
<body>
    ...
    <table class="tablelist">
		...
		<?php 
			...
			$xxoo="select count(*) from pl".$where1;
			// echo $xxoo;
			// exit;
			...
			$sql = "select goods.*,pl.* from goods,pl where goods.id=pl.goodsid".$where.$limit;
			...
			while($row = mysqli_fetch_assoc($result)){
				...
				echo "<td>{$row['hf']}</td>";
				echo "<td><a href='./update.php?huifu={$row['id']}' class='tablelink'>回复</a></td>";
				echo "</tr>";
				echo "</today>";
			}
			mysqli_close($link);
